...
Current evidence is in [STATUS.md](STATUS.md). Offline fixture completion is
...
```sh
python3 scripts/run_dsh_model.py --mode offline \
...
```sh
python3 scripts/run_dsh_model.py --mode prepare-live \
  --dsh-package /absolute/path/to/node_modules/@deepseek-ai/dsh \
...
```sh
python3 scripts/run_dsh_model.py --mode execute \
...
`scripts/prepare_native_docs_mini.py` freezes a small **supplied-context** slice:
...
The [2026-09-09 calibration](runs/native-evaluator-diagnostic-20260909/REPORT.zh.md)
...
node --experimental-loader ./scripts/dsh_native_loader.mjs \
  scripts/diagnose_native_evaluator.mjs \
...
Its actual [20-control calibration](runs/native-evaluator-v2-20260909/REPORT.zh.md)
...
evaluator. The [delta trace](runs/native-evaluator-v2-20260909/delta-explanation.json)
