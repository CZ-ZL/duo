...
DUO 是一个 DSH 插件：让你的 Agent 尝试修改，用你的测试比较原版和候选，
最后把结果与花费交给你检查。

你提供受支持的修改对象、评估器、必要的模型接入和预算。DUO 负责反复生成、
执行、比较和记录。原版保留，是否采用候选由你决定。

## 可以交给它什么任务

比如，你的 Agent 根据部署手册回答问题。你希望它给出文档中的正确命令，
附上来源，文档没有答案时就明确说没有。

[文档问答 starter](./dsh-plugin/QUICKSTART.zh-CN.md#真实任务-starter)
提供可编辑的提示词、手册、四道题、评估器和 provider 配置。你只需填写 DSH
...
- **接自己的测试。** 提供评估器，或包装已有测试函数。什么结果值得保留由你定义。
