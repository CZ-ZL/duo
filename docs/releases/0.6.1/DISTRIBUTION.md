...
Status: RELEASED as developer preview; exact-code CI passed. See the [distribution queue](../../product-foundation/DISTRIBUTION.md) for current delivery and marketplace status. The older RELEASE_NOTES.md and audit receipts remain historical snapshots.
...
[CI, archive checksum, anonymous download/install and catalog PR receipts](DISTRIBUTION_EVIDENCE.json) are recorded. The catalog PR remains subject to upstream review. Existing independent Caller evidence is earlier evidence; deterministic discovery/install checks do not prove that every Agent will choose DUO or that a marketplace will accept/rank it. Research remains paused and general method superiority unproven.
