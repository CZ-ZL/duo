...
npm run typecheck
node scripts/product/sync_product_docs.mjs --check
...
python3 scripts/product/verify_product_examples.py --package /path/to/installed/package --dsh-package "$DUO_DSH_PACKAGE" --output /tmp/new-duo-examples
```
