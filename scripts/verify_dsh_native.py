...
def main():
    ...
    source_hashes.update({f'scripts/{name}': sha(ROOT / 'scripts' / name)
    ...
    for profile_name, scenario, base_backed in scenarios:
        ...
        shutil.copyfile(ROOT / 'scripts' / ('dsh_adoption_host_driver.js' if scenario == 'adoption' else 'dsh_embedded_host_driver.js' if embedded_case else 'dsh_native_host_driver.js'), driver)
