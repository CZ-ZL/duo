...
def main():
    ...
    if live and result.returncode==0:
        inputs=[host.output/name for name in ['caller-input.json','pricing.json','parent-allocation.json']]+[profile/'dsh_caller_format_probe.js',ROOT/'scripts/run_caller_format_probe.py',source.resolve(),*map(Path,carry['sourceHashes'])]
