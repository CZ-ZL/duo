...
def main():
    ...
    if args.mode == 'execute':
        ...
        cmd = [sys.executable, str(ROOT / 'scripts/run_dsh_model.py'), '--mode', 'execute', '--output', str(args.output)]
    ...
    if live and result.returncode == 0:
        ...
        inputs += list((host.output / 'inputs').glob('*')) + [profile / 'schema-answer-evaluator.js', profile / 'dsh_maturation_host.js', profile / 'dsh_caller_host.js', profile / 'caller_diagnostics.mjs', profile / 'maturation_diagnostics.mjs', ROOT / 'scripts/run_maturation_calling.py']
        ...
        continuation_fields = {}
        if carry:
            inputs += [Path(k) for k in carry['sourceHashes']] + [ROOT / 'scripts/maturation_batch.py', ROOT / 'scripts/run_caller_format_probe.py']
