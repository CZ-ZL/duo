...
def main():
    ...
    if live and result.returncode==0:
        inputs=[host.output/f for f in ['pricing.json','PUBLIC_GUIDE.md','resources.json','experiment.json','input-provenance.json']]+list((host.output/'inputs').glob('*'))+[profile/f for f in modules+scripts]+[ROOT/'scripts/run_duo_caller.py',target]
