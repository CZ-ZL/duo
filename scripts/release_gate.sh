...
import sys
from scripts.research.code_evaluation import run_case
...
PY
run_check native env DUO_DSH_PACKAGE="$DSH_PKG" node --loader ./scripts/product/dsh_native_loader.mjs --test dsh-plugin/tests/native/*.test.js dsh-plugin/tests/*.test.js
run_check docs node scripts/product/sync_product_docs.mjs --check
...
run_check host python3 scripts/product/verify_dsh_native.py --dsh-package "$DSH_PKG" --output "$OUT/verify"
