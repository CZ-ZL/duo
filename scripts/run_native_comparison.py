...
def main():
    ...
    if args.mode in ['execute', 'inspect']:
        ...
        for row in protocol['arms']:
            ...
            cmd = [sys.executable, str(ROOT / 'scripts/run_dsh_model.py'), '--mode', 'execute', '--output', str(directory),
    ...
    for row in arms:
        cmd = [sys.executable, str(ROOT / 'scripts/run_dsh_model.py'), '--mode', args.mode, '--output', str(out / row['directory']),
    ...
    files = [out / 'comparison-protocol.json', *sorted((out / 'inputs').glob('*')),
             *[out / r['directory'] / 'prepared.json' for r in arms],
             ROOT / 'scripts/run_native_comparison.py', ROOT / 'scripts/prepare_native_comparison.py', ROOT / 'scripts/prepare_native_docs_mini.py']
