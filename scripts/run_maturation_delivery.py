...
def main():
    ...
    if process.returncode == 0 and live:
        ...
        inputs += [ROOT / 'scripts/run_maturation_delivery.py', ROOT / 'scripts/maturation_batch.py', profile / 'dsh_maturation_delivery.js', *map(Path, source_hashes), *map(Path, carry['sourceHashes']), args.pricing.resolve()]
