...
const manifest=JSON.parse(read('dsh-plugin/package.json')),cap=productCapabilities()
...
This page is generated from native/capabilities.js and package.json. Run scripts/sync_product_docs.mjs after an intentional support change. Public discovery uses the same catalog. Current acceptance is recorded in PRODUCT_ACCEPTANCE.json in the source distribution; historical experiment reports do not define current support.
...
Default public flow: describe → design (evaluate/optimize preset) → save an inspected contract → plan → authorized run → status/report. Advanced use adds screened warm history or replaces a provider in the profile. See AGENT_GUIDE.md and examples/product/README.md in the package.

Supported environment: ${cap.platform}. TypeScript semantic compilation and Windows/macOS are not currently verified. The text-hygiene example measures actual local text, not the behavior of an LLM Agent. Config execution requires a compatible executor; the persona executor is not a config executor.
...
for(const p of ['CURRENT_STATUS.md','dsh-plugin/CURRENT_STATUS.md']){
 if(process.argv.includes('--check')){if(read(p)!==text)throw new Error(p+' is stale; run node scripts/sync_product_docs.mjs')}
 else writeFileSync(new URL(p,root),text)
}
for(const name of ['AGENT_GUIDE.md','AGENT_REFERENCE.md']){
 if(process.argv.includes('--check')){if(read(name)!==read('dsh-plugin/'+name))throw new Error(name+' shipped copy drifted')}
