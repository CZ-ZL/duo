...
class Profile:
    ...
    def stage(self, name, entries, scripts, bundles=()):
        ...
        for file in scripts:
            shutil.copyfile(ROOT / 'scripts/research' / file, profile / file)
            self.sources['scripts/research/' + file] = sha(ROOT / 'scripts/research' / file)
...
def freeze_profile_inputs(host, profile, inputs):
    ...
    files = list(inputs) + [profile / 'package.json', profile / 'cordis.patch.yml',
        *[profile / 'node_modules/@dual-loop/dsh-plugin' / f for f in host.files],
        ROOT / 'scripts/research/run_dsh_model.py', ROOT / 'scripts/research/dsh_model_profile.py']
