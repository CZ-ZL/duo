...
def child_command(out, arm, mode, dsh=None, pricing=None):
    ...
    command=[sys.executable, str(ROOT/'scripts/research/run_dsh_model.py'), '--mode', mode,
    ...
    if code:
        ...
        modules=['scripts/research/dsh_code_evaluator.js','scripts/research/code_evaluation.py','scripts/research/code_worker.py','examples/native/method-feedback.js']
...
def main(kind='fact'):
    ...
    prepare_script='scripts/research/prepare_code_comparison.mjs' if kind=='code' else 'scripts/research/prepare_fact_comparison.mjs'
    prepare_command=['node','--loader','./scripts/product/dsh_native_loader.mjs',prepare_script,str(out),
    ...
    for path in [*sorted((out/'inputs').rglob('*')),Path(__file__),ROOT/prepare_script,
                 *([ROOT/'scripts/research/run_code_comparison.py'] if kind=='code' else [])]:
    ...
    if kind=='code' and args.search_profile=='properties-structured-v3':
        ...
        files[str((ROOT/'scripts/research/prepare_code_property_study.py').resolve())]=sha(ROOT/'scripts/research/prepare_code_property_study.py')
    for arm in protocol['arms']:
