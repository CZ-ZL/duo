...
export const Config = Schema.object({
  artifactRoot: Schema.string().required(),
  includeCallerSessions: Schema.boolean().default(false),
})
export function apply(ctx, config) {
  const root = resolve(config.artifactRoot),
    callerRequests = new Map()
  ctx.deepseekLlmApiExtensions.register('response_format', {
    prepare(request) {
      if (request.purpose) return
      const caller =
        config.includeCallerSessions && /^duo-caller-[a-f0-9-]{36}$/.test(request.sessionId ?? '')
      ...
      const filename = caller
        ? request.sessionId + '-' + requestIndex + '.json'
            ...
            JSON.stringify(
              {
                ...
                meaning: 'HTTP 2xx accepted; this is not proof of valid JSON or completed output.',
              },
              null,
              2,
            ) + '\n',
