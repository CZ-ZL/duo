...
export function describeFailure(error={},defaultComponent='duoController'){
 ...
 let component=defaultComponent,retryable=false,
  recoveryCondition='Inspect retained state and determine whether any work started or was charged',
  ...
  costState='UNKNOWN_UNTIL_LEDGER_INSPECTION',sideEffectState='UNKNOWN_UNTIL_EXECUTION_INSPECTION'
 if(code==='INVALID_ARGS'||code==='DUO_PLAN_CHANGED'){
  component=code==='INVALID_ARGS'?'dsh.tools.arguments':'duoController';retryable=true
  recoveryCondition=code==='INVALID_ARGS'?'Correct arguments to the discovered schema':'Inspect and accept the current plan within existing authorization'
  nextAction='Read the tool schema and dualloop_plan; verify target, providers and budget, then pass the exact planDigest'
  costState='NO_WORK_DISPATCHED_BY_THIS_CALL';sideEffectState='NO_WORK_DISPATCHED_BY_THIS_CALL'
 ...
 return {code,message:cause,cause,component:error.component??component,retryable,
  recoverability:retryable?'CORRECT_INPUT_THEN_REPLAN':'INSPECT_RETAINED_STATE_NO_AUTOMATIC_RETRY',
  recoveryCondition,nextAction:error.nextAction??nextAction,costState,sideEffectState}
}
