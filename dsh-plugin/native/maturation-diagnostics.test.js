...
import {diagnoseCombinedFailure,diagnoseRequestBoundary} from '../../scripts/maturation_diagnostics.mjs'
...
test('real trace input refusal is distinguished from a route mismatch without dispatch or retry',()=>{
 const failure=diagnoseRequestBoundary({kind:'caller',options:requestRoute,config,requestBytes:199405})
 ...
 assert.equal(diagnoseRequestBoundary({kind:'caller',options:requestRoute,config,requestBytes:196608}),null)
 assert.equal(diagnoseRequestBoundary({kind:'caller',options:requestRoute,config:{...config,callerMaxInputBytes:327680},requestBytes:199405}),null)
})
...
test('a larger Caller envelope never admits a wrong route or widens an inner request bound',()=>{
 ...
 for(const changed of [{provider:'other'},{model:'other'},{maxTokens:4096}]){
  const error=diagnoseRequestBoundary({kind:'caller',options:{...requestRoute,...changed},config:bigger,requestBytes:199405})
  assert.equal(error.code,'DUO_REQUEST_ROUTE_MISMATCH')
  assert.deepEqual(error.facts.routeMismatch,Object.keys(changed))
 }
 assert.equal(diagnoseRequestBoundary({kind:'inner',options:requestRoute,config:bigger,requestBytes:32769}).code,'DUO_REQUEST_INPUT_LIMIT')
})

test('a completed Caller turn with tool-shaped text is an incomplete experiment, not an executed tool',()=>{
 const failure=diagnoseCombinedFailure({failure:null,terminalReason:{kind:'completed'},finalText:'<｜｜DSML｜｜tool_calls><｜｜DSML｜｜invoke name="cordis_run">text only</｜｜DSML｜｜invoke></｜｜DSML｜｜tool_calls>',checks:{callerFinished:true,callerAttachedCustomEvaluator:false,twoCompletedNativeRuns:false,warmStartConsumed:false,interpretationValid:false,totalAccounting:true}})
 ...
 assert.deepEqual(failure.facts.failedChecks,['callerAttachedCustomEvaluator','twoCompletedNativeRuns','warmStartConsumed','interpretationValid'])
...
test('ordinary early completion names missing acceptance conditions without diagnosing a provider cause',()=>{
 const failure=diagnoseCombinedFailure({failure:null,terminalReason:{kind:'completed'},finalText:'Done.',checks:{callerFinished:true,twoReports:false,interpretationValid:false}})
 assert.ok(failure)
...
test('diagnostics preserve existing errors and do not reject a successful interpretation that quotes tool syntax',()=>{
 ...
 assert.equal(diagnoseCombinedFailure({failure:original,checks:{totalAccounting:false}}),original)
 assert.equal(diagnoseCombinedFailure({failure:null,checks:{callerFinished:true,interpretationValid:true},finalText:'A report may mention <｜｜DSML｜｜tool_calls> as data.'}),null)
})
