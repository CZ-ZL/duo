...
export const required = () =>
  fail('DUO_NOT_IMPLEMENTED', 'A DUO service provider must implement this method')
export class ContractService extends Service {
  constructor(ctx) {
    super(ctx, 'duoContract')
  }
  resolve() {
    return required()
  }
...
export class TargetService extends Service {
  ...
  projectDelta() {
    return fail(
      'DUO_HISTORY_UNSUPPORTED',
      'The configured Target does not provide a safe historical Delta projection',
    )
  }
}
export class ComparatorService extends Service {
  constructor(ctx) {
    super(ctx, 'duoComparator')
  }
  describe() {
    return required()
  }
...
for (const Base of [
  ...
  for (const key of Object.getOwnPropertyNames(Base.prototype))
    if (key !== 'constructor')
      Object.defineProperty(Base.prototype[key], abstractMethod, { value: true })
}
export function assertProviderMethods(service, component, methods) {
  for (const method of methods) {
    ...
    if (typeof descriptor?.value !== 'function' || descriptor.value[abstractMethod])
      throw Object.assign(
        new HarnessError(
          `Configured ${component} must implement ${method} as a concrete method before execution`,
          'DUO_PROVIDER_INTERFACE',
        ),
        { component },
      )
  }
