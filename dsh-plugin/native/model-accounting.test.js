...
import {priceUsage} from './model-call.js'

const cny = {id:'exact-decimal-control',currency:'CNY',inputCnyPerMillion:1,cacheReadCnyPerMillion:.02,outputCnyPerMillion:4}
const usage = (input,cache,output) => ({inputTokens:input,cacheReadTokens:cache,outputTokens:output,totalTokens:input+cache+output})

test('retained request2 does not acquire an extra nanoyuan from binary floating point',()=>{
 assert.equal(priceUsage(usage(682,1792,130),cny),.00123784)
})

test('recorded four-request usage agrees with exact decimal tariffs',()=>{
 for(const [input,cache,output,expected] of [[249,1664,167,.00095028],[682,1792,130,.00123784],[3206,2560,214,.0041132],[1341,5888,216,.00232276]])
...
test('nanoyuan ceiling is applied once after summing all token classes',()=>{
 const pricing={...cny,inputCnyPerMillion:.0004,cacheReadCnyPerMillion:.0004,outputCnyPerMillion:.0004}
...
test('scientific decimal tariffs retain the conservative ceiling',()=>{
 ...
 assert.equal(priceUsage(usage(10_000_000,0,0),{...cny,inputCnyPerMillion:1e-7}),.000001)
})

test('decimal CNY tariff grid equals an independent integer-token calculation',()=>{
 for(let i=0;i<512;i++){
  const input=i*17,cache=i*31,output=i%101
...
test('legacy USD retains its currency and exact tariff semantics',()=>{
 const pricing={id:'legacy-test',inputUsdPerMillion:1,cacheReadUsdPerMillion:.1,outputUsdPerMillion:2}
 assert.equal(priceUsage(usage(80,20,10),pricing),.000102)
})
