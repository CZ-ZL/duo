...
export function targetIdentity(adapter,snapshot){
 const identity=typeof adapter.identity==='function'?adapter.identity(snapshot):snapshot?.version
 if(typeof identity!=='string'||!identity)fail('DUO_TARGET_INVALID','Target must return a nonempty stable content identity/version')
 return identity
}
export function validHistorySnapshot(adapter,snapshot,parent){
 try{
  if(typeof adapter?.validateSnapshot!=='function'||!adapter.validateSnapshot(snapshot))return false
  ...
  return applied.version===snapshot.version&&targetIdentity(adapter,applied)===targetIdentity(adapter,snapshot)
 }catch{return false}
}
