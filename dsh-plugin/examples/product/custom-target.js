...
export async function apply(ctx) {
  class Target extends TargetService {
    ...
    projectDelta(d) {
      if (d?.kind !== 'replace-text' || typeof d.content !== 'string')
        fail('DUO_DELTA_INVALID', 'Expected replace-text content')
      return { kind: d.kind, content: d.content }
    }
    apply(c, p) {
      if (c.parentId !== p.id || c.parentVersion !== p.version)
        fail('DUO_DELTA_INVALID', 'Parent identity changed')
      const d = this.projectDelta(c.delta)
      return { ...c, content: d.content, version: digest(d.content) }
    }
  }
      ...
      return {
        ...
        candidates: Object.entries(quotas).flatMap(([mode, n]) =>
          Array.from({ length: n }, () => ({
            ...
            mode,
            family: 'trim',
            hypothesis: 'Remove trailing whitespace',
            delta: {
              kind: 'replace-text',
        ...
        artifact: {
          historyCandidateIds: (feedback.warmStart?.records ?? []).map((r) => r.source.candidateId),
        },
      }
    }
  }
  class Executor extends ExecutorService {
    ...
    async execute({ applied, signal }) {
      ...
      return { ...cost, artifact: { text: applied.content } }
    }
  }
  await ctx.plugin(Target)
  await ctx.plugin(Generator)
