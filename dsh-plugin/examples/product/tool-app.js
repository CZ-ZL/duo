  ...
  ctx.effect(() =>
    ctx.appReady.onReady(() => {
      void (async () => {
        ...
        let result
        if (request.tool === 'schemas')
          result = {
            isError: false,
            value: ctx.tools.schemas().filter((s) => s.name.startsWith('dualloop_')),
          }
        else {
          if (typeof request.tool !== 'string' || !request.tool.startsWith('dualloop_'))
            throw new Error('Only public dualloop tools are exposed by this example app')
          const abort = new AbortController()
          if (request.cancelAfterMs === 0) abort.abort()
          const timer =
            request.cancelAfterMs === undefined || request.cancelAfterMs === 0
              ? null
              : setTimeout(() => abort.abort(), request.cancelAfterMs)
          try {
            ...
            result = {
              ...
              error: value.error ? { message: value.error.message, info: value.error.info } : null,
        ...
        writeFileSync(
          config.responsePath,
          JSON.stringify({ isError: true, error: describeFailure(error, 'public-tool-app') }) +
            '\n',
          { flag: 'wx' },
        )
        ctx.appExit(1)
