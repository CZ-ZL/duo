...
test('bridge preserves structured preflight errors on exit 2', async () => {
  const result = await runCore('python3', coreDir, [
...
test('bridge forwards host cancellation without disguising it as a result', async () => {
  ...
  controller.abort()
  await assert.rejects(runCore('python3', coreDir, ['discover'], {
    ...options, signal: controller.signal,
  }), { name: 'AbortError' })
})

for (const handling of ['delayed', 'ignored', 'deadline']) test(`bridge waits for Python child using ${handling} termination`, async () => {
  const root = await mkdtemp(join(tmpdir(), 'duo-cancel-test-'))
  let pid
  try {
    await mkdir(join(root, 'dualloop'))
    await writeFile(join(root, 'dualloop', '__main__.py'), `
...
`)
    ...
    const pending = runCore('python3', root, [], {
      timeoutMs: handling === 'deadline' ? 200 : 4000, signal: controller.signal,
    })
    for (let index = 0; index < 100; index++) {
      try { pid = Number(await readFile(join(root, 'ready.pid'), 'utf8')); break }
    ...
    assert.throws(() => process.kill(pid, 0), { code: 'ESRCH' }, 'child must be gone when cancellation settles')
  } finally {
