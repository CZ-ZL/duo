...
export const Config = Schema.object({
  ...
  experiment: Schema.string().required()
  ...
  coreDir: Schema.string()
    .description('Absolute path of the dualloop repo root (child-process cwd). Omit if dualloop is pip-installed.'),
  python: Schema.string().default('python3')
    .description('Python executable used to launch the protocol core.'),
  journalDir: Schema.string()
    .description('Journal root for `status`. Defaults to <experiment dir>/journal, matching the core CLI.'),
  runTimeoutMs: Schema.number().default(30 * 60 * 1000)
...
export function apply(ctx, config) {
  ...
  for (const [tool, args, description] of [
    ['dualloop_discover', ['discover'], 'Discover implemented DualLoop capabilities, contracts, permissions and limitations. No experiment execution.'],
    ['dualloop_plan', ['plan', '--experiment', experiment, '--journal-dir', journalDir],
  ...
  ctx.tools.register(defineTool({
    ...
    isConcurrencySafe: () => true,
    async execute(_args, exec) {
      const { stdout } = await runCore(
        ['status', '--journal-dir', journalDir],
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      generations: {
        type: 'number',
        description: 'Override the number of fast-loop generations (default: experiment file value).',
      },
      plan_digest: { type: 'string', description: 'Digest from dualloop_plan, required for Agent-native runs.' },
    },
    output: {
      schema: { type: 'string', description: 'Human-readable run summary from the protocol core' },
      render: (_args, value) => [{ type: 'text', text: value }],
    },
