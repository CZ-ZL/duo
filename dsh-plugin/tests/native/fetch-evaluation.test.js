...
test('exact evaluator distinguishes correct, wrong and unsupported answers', () => {
  ...
  assert.equal(
    scoreAnswer(JSON.stringify({ q: { type: 'string', quote: 'limit?: number' } }), tasks, body)
      .passed,
    0,
  )
  assert.equal(scoreAnswer(good, tasks, body.slice(0, 20)).passed, 0)
  assert.equal(
    scoreAnswer(JSON.stringify({ q: { type: 'UNKNOWN', quote: '' } }), tasks, body).status,
    'VALID',
  )
})
test('exact evaluator rejects duplicate keys and extra narrative without partial credit', () => {
  assert.equal(
    scoreAnswer('{"q":{"type":"number","type":"number","quote":"limit?: number"}}', tasks, body)
      .status,
    'INVALID_OUTPUT',
  )
  assert.equal(scoreAnswer(good + ' This is correct.', tasks, body).status, 'INVALID_OUTPUT')
  assert.equal(
    scoreAnswer('{"q":{"type":"number","quote":"limit?: number","extra":1}}', tasks, body).status,
    'INVALID_OUTPUT',
  )
})
