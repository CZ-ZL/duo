...
import { TopKGate } from '../../native/policies.js'
const budget = {
  currency: 'CNY',
  maxCostCny: 0,
  maxSessions: 100,
...
test('multiple policy-eligible candidates produce one rank-selected champion and consistent Journal', async (t) => {
  ...
  const { buildReport } = await import('../../native/observer.js')
  ...
  const { strategyFeedback } = await import('../../native/evidence-strategy.js')
