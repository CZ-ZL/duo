...
async function setup(
  t,
  {
    ...
    stageCaps = { unit: 10, benchmark: 6, holdout: 5, audit: 4 },
    generations = 2,
  } = {},
) {
  const root = mkdtempSync(join(tmpdir(), 'duo-ladder-')),
  ...
  const contract = {
    ...
    searchStages: TIERS.map((tier, i) => ({
      ...objective(tier),
      tier,
      purpose: i === 0 ? 'screen' : i === TIERS.length - 1 ? 'confirm' : 'rank',
      informationGain: 'Declared credibility level ' + tier,
      maxEvaluations: stageCaps[tier],
      topK: i === TIERS.length - 1 ? 0 : 1,
    })),
...
test('custom ladder: cascade order, stage attempts, pool accounting and observer columns', async (t) => {
  ...
  for (const id of new Set(evaluated.map((x) => x.split(':')[0]))) {
    ...
    assert.ok(
      seen.every((tier, i) => !i || order.indexOf(tier) > order.indexOf(seen[i - 1])),
      id + ' tier order is monotonic: ' + seen.join(','),
    )
  }
  assert.deepEqual(Object.keys(result.stageAttempts), TIERS)
  assert.ok(
    result.stageAttempts.unit <= 10 &&
      result.stageAttempts.benchmark <= 6 &&
      result.stageAttempts.holdout <= 5 &&
      result.stageAttempts.audit <= 4,
  )
  ...
  assert.equal(
    result.budget.slowAttempts,
    result.stageAttempts.benchmark +
      result.stageAttempts.holdout +
      result.stageAttempts.audit +
      result.final.length,
  )
  ...
  const candidate = report.candidates.find(
    (c) => c.id !== 'baseline' && c.unit.state === 'EVALUATED',
  )
  ...
  assert.deepEqual(
    [
      ...new Set(
        status.events.filter((e) => e.kind === 'gate').flatMap((e) => [e.fromTier, e.toTier]),
      ),
    ],
    TIERS,
  )
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...contract,
      id: 'ladder-warm',
      warmStart: { runIds: [p.runId], fixturePolicy: 'ideas_only' },
    }),
  )
  ...
  assert.ok(
    warm.warmStart.context.records.every((r) =>
      r.source.evaluators.every((e) => TIERS.includes(e.tier)),
    ),
  )
})
test('a stage evaluation cap clamps gate promotion at the frozen ladder boundary', async (t) => {
  ...
  class Counted extends FixtureExecutor {
    async execute(args) {
      evaluated.push(args.tier)
      return super.execute(args)
    }
  }
  const { ctx } = await setup(t, {
    Executor: Counted,
    stageCaps: { unit: 10, benchmark: 10, holdout: 10, audit: 1 },
    generations: 1,
  })
  const p = ctx.duoController.plan(),
    result = await ctx.duoController.run({ planDigest: p.planDigest })
