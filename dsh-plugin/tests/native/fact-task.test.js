...
import {
  ...
  buildFactTask,
  measureFactBatch,
} from '../../../examples/native/fact-task.js'
...
const expected = {
  ...
  unit: null,
  abstain: false,
  support: [{ source: 'release', line: 2 }],
  allowedSources: ['release', 'draft'],
}
const answer = {
  answer: '2024-06-30',
  unit: null,
  abstain: false,
  citations: ['release'],
  evidence: [{ source: 'release', line: 2 }],
}
test('same-shaped controls distinguish wrong facts and allowed but unsupported citations', () => {
  ...
  const wrongSource = judgeFactAnswer(expected, {
    ...answer,
    citations: ['draft'],
    evidence: [{ source: 'draft', line: 2 }],
  })
  assert.equal(good?.correct, true)
  assert.equal(good.supported, true)
  assert.equal(wrongYear.formatValid, true)
  assert.equal(wrongYear.correct, false)
  assert.equal(wrongYear.supported, false)
  assert.equal(wrongSource.formatValid, true)
  assert.equal(wrongSource.correct, true)
  assert.equal(wrongSource.supported, false)
...
test('normalization accepts equivalent units but not wrong magnitudes or missing conflict evidence', () => {
  ...
  const conflict = {
    ...
    abstain: true,
    support: [...expected.support, { source: 'draft', line: 2 }],
  }
  const abstention = {
    ...answer,
    answer: 'INSUFFICIENT_EVIDENCE',
    abstain: true,
    citations: ['release', 'draft'],
    evidence: conflict.support,
  }
  assert.equal(judgeFactAnswer(conflict, abstention).supported, true)
  assert.equal(
    judgeFactAnswer(conflict, { ...abstention, evidence: expected.support, citations: ['release'] })
      .supported,
    false,
  )
...
test('malformed output and unbounded or invented values do not pass validity', () => {
  for (const bad of [
    ...
    [],
    { ...answer, answer: '' },
    { ...answer, evidence: [{ source: 'release', line: -1 }] },
    { ...answer, citations: 'release' },
  ])
...
test('synthetic task binds native receipt identity and keeps answer keys outside execution inputs', () => {
  ...
  assert.deepEqual(
    ['fast', 'slow', 'final'].map((t) => dataset[t].tasks.length),
    [5, 10, 10],
  )
  const groups = Object.values(manifest.groups).flat()
  assert.equal(new Set(groups).size, groups.length)
  const sources = new Set()
  for (const tier of ['fast', 'slow', 'final'])
    for (const task of dataset[tier].tasks) {
  ...
  const unit = dataset.fast.tasks[1],
    date = dataset.fast.tasks[2]
...
test('batch measurement retains planned denominator for bad or missing answers and refuses crossed receipts', () => {
  const { dataset, answerKey } = buildFactTask(),
    candidate = { id: 'baseline', version: 'unchanged' }
  ...
  const measure = (a) =>
    measureFactBatch(dataset, answerKey, { candidate, artifact: a, tier: 'fast' })
  assert.deepEqual(measure(artifact).metrics, {
    supported_accuracy: 1,
    fact_accuracy: 1,
    format_valid: true,
    sample_size: 5,
  })
  ...
  assert.equal(missing.metrics.supported_accuracy, 0.8)
  assert.equal(missing.metrics.sample_size, 5)
  assert.equal(missing.metrics.format_valid, false)
  const malformed = measure({ ...artifact, text: 'not json' })
  assert.equal(malformed.metrics.supported_accuracy, 0)
  assert.equal(malformed.metrics.sample_size, 5)
  ...
  tampered.datasetDigest = 'other'
  assert.throws(
    () => measureFactBatch(dataset, tampered, { candidate, artifact, tier: 'fast' }),
    /mismatch/,
  )
})
